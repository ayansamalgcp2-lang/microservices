...
module "vpc" {
  source       = "../../modules/vpc"
  project_id   = var.project_id
  network_name = "${var.project_prefix}-dev-vpc"
  subnets = [
    { name = "dev-priv-a", ip_cidr_range = var.subnet_cidr_a, region = var.region },
    { name = "dev-priv-b", ip_cidr_range = var.subnet_cidr_b, region = var.region }
  ]
}
...
module "connector" {
  ...
  name   = "dev-cr-conn"
  region = var.region
  subnet = module.vpc.subnets["dev-priv-a"].id
}
...
module "sql" {
  ...
  db_name    = "${var.project_prefix}_dev"
}
