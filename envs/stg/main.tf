...
module "vpc" {
  source       = "../../modules/vpc"
  project_id   = var.project_id
  network_name = "${var.project_prefix}-stg-vpc"
  subnets = [
    { name = "stg-priv-a", ip_cidr_range = var.subnet_cidr_a, region = var.region },
    { name = "stg-priv-b", ip_cidr_range = var.subnet_cidr_b, region = var.region }
  ]
}
...
module "connector" {
  ...
  name   = "stg-cr-conn"
  region = var.region
  subnet = module.vpc.subnets["stg-priv-a"].id
}
...
module "sql" {
  ...
  db_name    = "${var.project_prefix}_stg"
}
